...
import type { ClientAnalysisDonutChartProps } from "./types";
import { labelFormatter, tooltipFormatter } from "./formatter";

const ClientAnalysisDonutChart = ({
  clientTypeSeries,
}: ClientAnalysisDonutChartProps) => {
  const themeStyles = [
    {
    ...
    series: [
      {
        name: "Client type",
        innerSize: "75%",
        data: clientTypeSeries.map((client, idx) => ({
          name: client.name,
          y: client.y,
...
export default ClientAnalysisDonutChart;
